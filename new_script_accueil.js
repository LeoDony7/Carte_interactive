...
document.addEventListener('DOMContentLoaded', function() {
     ...
     * Redirige vers l'éditeur en mode création
     */
    ...
    function openMapsList() {
        ...
        // Vérifier s'il y a des cartes sauvegardées
        try {
            const savedMaps = JSON.parse(localStorage.getItem('mapCreator_maps') || '[]');
            
            if (savedMaps.length === 0) {
                // Aucune carte sauvée, proposer de créer la première
                const shouldCreate = confirm(
                    'Vous n\'avez encore aucune carte sauvegardée.\n\n' +
                    'Voulez-vous créer votre première carte maintenant ?'
                );
            ...
            console.warn('Erreur lors de la vérification des cartes:', error);
        }
        
        // Redirection vers la liste des cartes
        window.location.href = 'map-list.html';
    }
